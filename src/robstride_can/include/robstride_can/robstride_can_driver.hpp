 ...
 * 
 * 支持 RS00 与 EL05 系列电机（私有协议）
 */

#ifndef ROBSTRIDE_CAN__ROBSTRIDE_CAN_DRIVER_HPP_
#define ROBSTRIDE_CAN__ROBSTRIDE_CAN_DRIVER_HPP_
...
public:
   ...
   * @param motor_id 电机 CAN ID
   * @param param_index 参数索引
   * @param value 参数值 (uint32_t)
   * @return 成功返回 true
   */
  bool writeParameter(uint8_t motor_id, uint16_t param_index, uint32_t value);
   ...
   * @brief 设置速度/位置模式电流限制
   * @param motor_id 电机 CAN ID
   * @param current_limit 最大相电流 (A)
   * @return 成功返回 true
   */
  bool setCurrentLimit(uint8_t motor_id, double current_limit);

  /**
   * @brief 设置位置环 Kp
   * @param motor_id 电机 CAN ID
   * @param position_kp 位置环 Kp
   * @return 成功返回 true
   */
  bool setPositionKp(uint8_t motor_id, double position_kp);

  /**
   * @brief 设置速度环 Kp
   * @param motor_id 电机 CAN ID
   * @param speed_kp 速度环 Kp
   * @return 成功返回 true
   */
  bool setSpeedKp(uint8_t motor_id, double speed_kp);

  /**
   * @brief 设置速度环 Ki
   * @param motor_id 电机 CAN ID
   * @param speed_ki 速度环 Ki
   * @return 成功返回 true
   */
  bool setSpeedKi(uint8_t motor_id, double speed_ki);

  /**
   * @brief 设置速度滤波系数
   * @param motor_id 电机 CAN ID
   * @param filter_gain 速度滤波系数
   * @return 成功返回 true
   */
  bool setSpeedFilterGain(uint8_t motor_id, double filter_gain);
...
#endif  // ROBSTRIDE_CAN__ROBSTRIDE_CAN_DRIVER_HPP_
