...
constexpr uint16_t PARAM_SPD_FILT_GAIN = 0x7021;
  ...
  , receive_running_(false)
{
  ...
  motor_types_.resize(256, MotorType::RS00);
...
uint16_t RobstrideCanDriver::floatToUint16(double x, double x_min, double x_max)
{
  ...
  return static_cast<uint16_t>((x - x_min) * 65535.0 / span + 0.5);
}
  ...
  double torque)
{
  ...
  if (verbose_ && debug_count++ % 60000 == 0) {
...
bool RobstrideCanDriver::writeParameter(uint8_t motor_id, uint16_t param_index, uint32_t value)
{
  can_frame frame;
  std::memset(&frame, 0, sizeof(frame));
...
{
  return writeParameter(motor_id, PARAM_RUN_MODE, static_cast<uint32_t>(static_cast<uint8_t>(mode)));
}
...
bool RobstrideCanDriver::setCurrentLimit(uint8_t motor_id, double current_limit)
{
  return writeParameter(motor_id, PARAM_LIMIT_CUR, static_cast<float>(current_limit));
}

bool RobstrideCanDriver::setPositionKp(uint8_t motor_id, double position_kp)
{
  return writeParameter(motor_id, PARAM_LOC_KP, static_cast<float>(position_kp));
}

bool RobstrideCanDriver::setSpeedKp(uint8_t motor_id, double speed_kp)
{
  return writeParameter(motor_id, PARAM_SPD_KP, static_cast<float>(speed_kp));
}

bool RobstrideCanDriver::setSpeedKi(uint8_t motor_id, double speed_ki)
{
  return writeParameter(motor_id, PARAM_SPD_KI, static_cast<float>(speed_ki));
}

bool RobstrideCanDriver::setSpeedFilterGain(uint8_t motor_id, double filter_gain)
{
  return writeParameter(motor_id, PARAM_SPD_FILT_GAIN, static_cast<float>(filter_gain));
}
  ...
  while (receive_running_) {
    if (receiveFrame(frame, 10)) {
      ...
      if (frame.can_id & CAN_EFF_FLAG) {
        ...
        if (comm_type == 2 && motor_id < motor_types_.size()) {
          // 统计反馈
...
void RobstrideCanDriver::parseFeedback(const can_frame& frame, MotorType motor_type)
{
  ...
  if (motor_id >= motor_feedbacks_.size()) return;
  ...
  if (verbose_ && motor_id == 3 && ++fb_debug_count % 60000 == 0) {
